...
- spawn_agents brings in 1–8 temporary task lanes, each with one assignment and a worktree placement; neutral lane names are assigned automatically. A worker has no persona and no durable memory. It exists only for its current assignment and must be closed when that assignment is complete.
