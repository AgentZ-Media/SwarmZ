...
- 🛰️ **Fleet overview** — `⌘E` zooms out to every workspace at once, live: real terminals scaled down, not snapshots, each pane with its own status/branch/context chrome. Workspaces with an agent that needs you carry a static amber `⚑` border; drive it fully from the keyboard (arrows move the selection, Enter jumps in, Tab cycles the panes that need you — oldest first, Home/End first/last) or click any pane to jump straight to it. The perfect second-monitor view while a dozen agents work.
- 🤖 **Orchestrator** — `⌘⇧O` opens a chat sidebar with an AI team lead over the whole fleet: it inspects panes and transcripts, checks git status, discovers your projects, prompts agents and spins up new panes, worktrees or workspaces via tools — and pings you (and its own context) when an agent it tasked finishes. Two interchangeable brains (Settings → Orchestrator): **Codex** via your ChatGPT login, or **OpenRouter** with your own key and any tool-calling model; each chat keeps the brain it was started with. Guard rails included: an optional review mode pastes prompts without submitting them, and a busy policy can refuse prompts to panes that are still working.
- 🌈 **Vibe Mode** — `⌘⇧V` swaps the terminal grid for a second view built around **native Codex sessions**: agents driven directly through the Codex app-server instead of a PTY, so their work arrives as structured cards — streaming assistant messages (rendered as markdown), collapsible command output, syntax-highlighted **diff cards**, plans, and **approval prompts** you allow or decline right in the composer (`⏎` allow · `⎋` decline). A left rail lists every session with its live signal (working / needs you / finished / idle) and diff counter; the stage is **Orchestrator-first** — the pinned Conductor card puts the orchestrator chat front and center, and `@session` in the composer routes a message straight to one session. Start a session on any folder (a git worktree folder works too), pick its model, effort and access (`workspace-write` or full), and the orchestrator can create and prompt sessions as a second agent species alongside grid panes.
- 🧑‍🔬 **Custom agents** — build your own specialists in conversation. `⌘K → "New agent"` names one and hands off to a chat-driven **Builder** that asks a few questions and writes the agent's files live — a persona (`soul.md`), a starter `memory.md`, optional `knowledge/` — into a folder under `~/.swarmz/agents/` that stays hand-editable and is the single source of truth. Each agent carries its own voice, memory and knowledge, and **maintains its own memory itself** — it appends durable facts to `memory.md` as it learns them (approval-free, even when it's working in one of your projects). Start one anywhere: as a terminal pane, a native Vibe session, or by asking **Maestro** — "let the YouTube coach take this" — and the orchestrator lists your agents and starts the right specialist with its persona and defaults already loaded. Manage them in the **Agents** library (`⌘K → "Agents"`); the accent color is pure identity, never status.
...
- 🎛️ **Profiles** — presets for runtime, startup command, flags and default working directory, persisted across restarts. Codex ships with standard YOLO/full-access, workspace and read-only profiles; new agents prefill the profile's default folder, or the last folder you used.
...
- ⚙️ **Settings** — `⌘,` (or the gear in the title bar) opens a settings window: session restore, workspace presets, custom commands, voice dictation (engine cloud/local incl. local-model download, OpenRouter key, hotkey mode, auto-submit, cleanup model & prompt), orchestrator (brain codex/OpenRouter, model, prompt auto-submit, busy policy, project scan folders), default app for new panes (fresh installs default to Codex full-access/YOLO mode), default terminal font size, default startup command, path overrides for the `claude`, `codex` and `git` binaries, update controls and an About panel.
...
| `⌘⇧O` | Orchestrator chat sidebar |
| `⌘⇧V` | Toggle Vibe Mode (native Codex sessions ↔ terminal grid) |
| `⌘E` | Fleet overview (all workspaces live) |
| `⌘⇧A` | Jump to the next agent waiting for input |
| `⌘1` … `⌘9` | Switch workspace |
| `⌘⇧[` / `⌘⇧]` | Previous / next workspace |
| `⌘[` / `⌘]` | Previous / next pane in the active workspace |
| `⌘⇧N` | New workspace |
...
- Everything is local: PTYs, usage parsing and persistence all happen on your machine. SwarmZ's only own network requests are the key-less OpenRouter pricing/model catalog, the Anthropic usage endpoint for Claude plan limits (authenticated with your local Claude Code login; the token never leaves your machine except to Anthropic), and — only when you use voice dictation with the Cloud engine — your recordings to the OpenRouter transcription/chat endpoints, authenticated with your own key from the Keychain. The orchestrator talks to the model you chose: the Codex brain goes through your local `codex` CLI (your ChatGPT login), the OpenRouter brain sends the chat (including fleet status and tool results) to OpenRouter with your own key. Codex usage/limits are read from local Codex session files. With the Local engine recordings never leave the machine (the one-time model download comes from Hugging Face).

## 🤝 Contributing

Issues and PRs welcome. Before submitting:

- `./node_modules/.bin/tsc --noEmit` and `pnpm test` (vitest) must pass; `cargo test` for Rust changes.
