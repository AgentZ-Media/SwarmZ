...
export function MissionRecoveryPanel({ missionId, open }: { missionId: string; open: boolean }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) close(); }}>
      <DrawerContent className="w-[min(620px,95vw)]">
        <header className="flex min-h-14 items-center gap-3 border-b border-line px-4">
          <ShieldAlert size={15} className={active.length ? "text-attn" : "text-ok"} aria-hidden />
          <div><DialogTitle className="text-14">Recovery &amp; delivery ledger</DialogTitle><DialogDescription className="mt-0.5 text-10">Write-ahead effects, durable claims and restart reconciliation</DialogDescription></div>
          <button onClick={close} aria-label="Close recovery panel" className="focus-ring ml-auto flex h-8 w-8 items-center justify-center rounded-md text-fnt hover:bg-card hover:text-txt"><X size={14} aria-hidden /></button>
        </header>
